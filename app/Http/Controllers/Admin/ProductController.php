   ...
   public function list(Request $request): View|Factory|Application
{
    ...
    if ($request->has('search')) {
        ...
        $query = $this->product->where(function ($q) use ($key) {
            foreach ($key as $value) {
                $q->orWhere('id', 'like', "{$value}%")  // Match starting characters of ID
                    ->orWhere('name', 'like', "{$value}%"); // Match starting characters of name
            }
        })->latest();
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        if (!empty($request->file('images'))) {
            foreach ($request->images as $img) {
                ...
                $imageNameToSave = \Carbon\Carbon::now()->toDateString() . "-" . uniqid() . "." . $imageExtension;
                ...
                Storage::disk('public')->put($dir . $imageNameToSave, file_get_contents($img));
     ...
     * @return RedirectResponse
     */
   public function removeImage($id, $name): JsonResponse
{
    $product = $this->product->find($id);
    $imageArray = [];
    ...
    if (count($imageArray) == 0) {
        return response()->json([
            'success' => false,
            'message' => translate('Product must have at least one image!') // Only one warning message
        ]);
    }
    ...
    return response()->json([
        'success' => true,
        'message' => translate('Image removed successfully!') // Only one success message
    ]);
}
